...
namespace hqu
{
	...
	public:
		...
		vector<T> operator+(const std::vector<std::string> &rhs) const;
		//Task 3 - operator=
	...
	template <typename T>
	vector<T> vector<T>::operator+(const std::vector<std::string> &rhs) const
	{
		vector<T> result(this->size() + rhs.size());
		for (int i = 0; i < this->size(); ++i)
			result.push_back((*this)[i]);
		for (int i = 0; i < rhs.size(); i++)
		{
			char * temp = new char[rhs[i].size()+1];
			 	// std::cout<<"rhs[i].size()"<<rhs[i].size()<<std::endl;
			...
			*(temp+rhs[i].size())='\0';
			hqu::string hquTemp(temp);
			result.push_back(hquTemp);
			delete[] temp;
		}
		return result;
	}
